...
const ReviewService = {
  ...
  getOneReview: async (reviewId: string): Promise<Review> => {
    const res = await axiosInstance.get(`/reviews/review/${reviewId}`);
    return res.data;
  },
  ...
  updateReview: async (
    ...
    const res = await axiosInstance.put(`/reviews/review/${reviewId}`, data);
    return res.data;
  },

  deleteReview: async (reviewId: string): Promise<{ message: string; review: Review }> => {
    const res = await axiosInstance.delete(`/reviews/review/${reviewId}`);
    return res.data;
  },
};
